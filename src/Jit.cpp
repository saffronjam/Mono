...
namespace Mono
{
static void OnLogCallback(const char* log_domain, const char* log_level, const char* message, mono_bool /*fatal*/,
                          void* /*user_data*/)

{
	...
	format_msg += message;
	const auto& logger = GetLogHandler(category);
	if (logger)
	{
		logger(format_msg);
	}
	...
	{
		const auto& default_logger = GetLogHandler("default");
		if (default_logger)
		{
...
{
	mono_set_assemblies_path(mono_path.c_str());
	...
	SetLogHandler("default", [](const std::string& msg) { std::cout << msg << std::endl; });
